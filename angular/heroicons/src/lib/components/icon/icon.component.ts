...
@Component({
  ...
  template: `
    <svg
      ...
      [attr.stroke]="_type === 'solid' ? null : 'currentColor'"
      [attr.fill]="_type === 'solid' ? 'currentColor' : 'none'"
      #svgRef
    ></svg>
  `,
...
export class IconComponent implements AfterViewInit, OnDestroy {
  ...
  private get __needsToRender() {
    if (!this._isAfterViewInit || !this._name || !this._type) return false

    return (
      !this._currRendered ||
      !(this._currRendered.name === this._name && this._currRendered.type === this._type)
    )
  }
